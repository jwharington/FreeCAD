...
The ConstraintJig321 provides a 3-2-1 kinematic grounding of the mesh
so the static FEM solve is well-posed. The ConstraintReaction applies
the bearing forces that the Assembly dynamics solver would compute.
...
def setup(doc=None, solvertype="ccxtools", exercise_loadcases=True):
	...
	con_jig.LinearAcceleration = FreeCAD.Vector(-177, 0, -9633)
	con_jig.LinearVelocity = FreeCAD.Vector(0, 0, 0)
	con_jig.AngularVelocity = FreeCAD.Vector(0, 1, 0)
	...
	con_reaction.Force = FreeCAD.Vector(-2, 0, 99)
	con_reaction.Torque = FreeCAD.Vector(0, 17000, 0)
