...
def _evaluate_split_strategy_attempt(shape, strategy):
    ...
    if non_planar_result is not None and non_planar_result["status"] != "NotImplemented":
        ...
        # surface and both mould halves directly; the split is intrinsic to
        # the construction.
        parting = {
        ...
        # The C++ solver produced no result (binding unavailable, or a
        # non-ready solver status). Surface the failure — the planar fallback
        # has been removed; everything must go through the non-planar solver.
        failure_summary = (non_planar_result or {}).get(
