...
- **Known limitation (NOT a G7 blocker):** hiding the native Part shape removes FreeCAD's selection highlight (blue on hover, green on select) — the highlight has no surface to render on, and the shader overlay in `DrapeHost` is not wired to receive selection highlight. Tracked as a polish item.
