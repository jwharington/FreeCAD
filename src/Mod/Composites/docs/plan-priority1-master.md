...
Current state: the drape-mesh branch is removed, the shader attaches to the **SupportSurface** geometry (MCP-verified), the native Part shape is hidden via the 'Grid' display mode, ConicalPanelSupport is hidden, GLSL compiles/links clean, the grid logic is fixed, and out-of-grid UV extrapolation works. Persistence (save/reload .FCStd) preserves drape state and re-attaches the shader — `test_g7_persistence.py` all 8 checks pass. **Selection highlight on the shader overlay is now working** (2026-07-16) via two `SoShaderParameter` uniforms (`sel_color` vec3 + `sel_state` 1f) driven by the VP's `SelectionObserver` callbacks — green on select, blue on hover, neutral grey otherwise; proven by an opaque-pass GPU readback. **P4 `offset_angle` resolved** (2026-07-16): grid rotation implemented in GLSL + plumbing bugs fixed (int/str key mismatch, reload-clobbering). **Dead persistence path cleaned up** (2026-07-16): removed `_RehydratedBackend` class (~259 lines) + `_rehydrate` method (~44 lines) — both were dead (never called; the JSON properties they referenced were declared nowhere). Persistence works via re-solve-on-restore (`execute()` → C++ backend); verified MCP: 887 tex_coords identical before save → after reload. **Reload duplicate-surface (flashing) fixed** (2026-07-16): on reload, `attach()` ran `update_visibility` *before* the shader attached (`has_shader=False` → `DisplayMode="Shaded"`), and nothing re-ran it after the shader attached during recompute — so the native Part shape (`ModeSwitch → FlatRoot → SoBrepFaceSet`) stayed visible alongside the shader's `SupportSurface` → duplicate cone → flashing. Fix: `_inject_drape_geometry` now calls `vp.Proxy.update_visibility(vp)` after `reload_shader()`, so `DisplayMode` flips to `"Grid"` (`whichChild` → `GridEmptyRoot`) once the shader is attached. Verified MCP: fresh build AND reload both show `DisplayMode="Grid"`, `whichChild=0` (native shape hidden); only the shader `SupportSurface` renders. (Pre-existing reload timing bug, not caused by the dead-code removal — exposed by running a reload test.) **G0/G8 closed**: all 21 Composites test modules green (296 tests, 0 failures).
