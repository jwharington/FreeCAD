...
- [x] Investigate candidate non-planar parting-surface models and choose the best one for twisted geometry.
  ...
  - Model selected and spec written: see `docs/non-planar-parting-requirements.md` for the full requirements (C++ in nextdrape, pybind11 binding, part-line = `normal·D=0` equator via `Contap_Contour`/`reflectLines`, tangent-surface degenerate → `D`-midpoint, skirt projected outward `D`-height-preserving to the rectangular block edge, convex 3D-outset path kept as planning preview, general BREP shell-half path via `BRepFeat_SplitShape::SplitByWire` for twisted/non-convex). Construction API references verified against OCCT 8; genuinely uncertain steps flagged *(needs verification)* for the C++ author.
  - Blocked: implementation is a separate, larger C++ effort in the `nextdrape` submodule (nextdrape has its own debugging work in progress); the Python side (`analyze_source_shape`) is already ready to consume a non-planar parting surface behind the WC gate once the C++ helper lands.
