...
- [x] Add direct regression checks or diagnostic hooks for each suspected fault source:
  ...
  - candidate-direction heuristic mismatch: a crafted asymmetric solid must prove the best direction wins for geometric evidence, not bbox extent.
    - Evidence: `TestCandidateDirectionRanking.test_equal_extent_ranks_lower_backface_axis_first` uses a right-triangular prism with equal X/Y/Z extents (10) but different backface ratios: +Z (small triangular backface, ratio 0.113) ranks above +X (large rectangular backface, ratio 0.227) on geometric evidence alone, with extents tied. This proves the `bbox_score * (1 - 0.25·backface_ratio)` score weighs geometry, not just extent.
    - Finding (extent-dominance): with `GEOMETRY_BACKFACE_WEIGHT = 0.25`, geometry can only penalize a candidate by 25%, so the better-releasing axis beats the shortest-extent axis ONLY when extents are nearly equal. A prism with X=10, Z=12 (Z lower backface but larger extent) still ranks X first — extent dominates. This is already pinned by the existing `test_tall_thin_box_picks_smallest_extent_axis` / `test_flat_wide_box_picks_z` tests (with a design-question comment). Raising the weight so better-release can overcome extent differences is an open design decision, not made here.
